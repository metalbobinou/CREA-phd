...
def GenerateCAHparameter(filename, NbClusters):
    outfile="NULL"
    separator=";"
    XSize=10
    YSize=9
    ...
    ## Choix de la taille à indiquer sur le graphique : NbClusters
    threshold_height = NbClusters
    ...
    plt.savefig('CAH-MaxClust-' + str(NbClusters) + "-" + outfile + '.png', format='png', bbox_inches='tight') # PNG
    ...
    ## Print in a file
    print("## Clusters CSV")
    CSV_OUT_filename = 'Clusters-CAH-MaxClust-' + str(NbClusters) + "-" + outfile + '.csv'
...
def TwoArgs():
    printHelp()
def ThreeArgs():
    GenerateCAHparameter(sys.argv[1], int(sys.argv[2]))
