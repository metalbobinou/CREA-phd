...
# Simply : Dissimilarity = 1 - Similarity
def SimpleDissimilarity(SimMatrix):
    #df.apply(lambda x : (x-x.min())/(x.max()-x.min()))
    DissimMatrix = SimMatrix.copy()
    DissimMatrix.apply(lambda x : (1 - x))
    return (DissimMatrix)



def CalculateDissimilarityMatrix(SimMatrix):
    DissimMatrix = SimpleDissimilarity(SimMatrix)
    return (DissimMatrix)
...
def GenerateMDS(filename, outfile="NULL", separator=";",
    ...
    SimMatrix = data.copy()
    DissimMatrix = CalculateDissimilarityMatrix(SimMatrix)
