...
def GenerateOPTICS(filename, outfile="NULL", separator=";", XSize=10, YSize=9, dpi=300):
    ...
    param_k = 2             # (dimension * 2) - 1 || [Default at 2 in example]
    param_min_samples = 5   # k + 1 || Best to use is 4 for 2D (or 5 ?)
    #param_xi =
    ...
    plt.plot(distances)
    ## Save into a PNG
    plt.savefig('NearestNeighbors-OPTICS-' + outfile + '.png', format='png', bbox_inches='tight') # PNG
    ...
    clust = OPTICS(min_samples=param_min_samples, metric="precomputed")
    ...
    if (False):
        ...
        ax2.plot(data[clust.labels_ == -1, 0],
                 data[clust.labels_ == -1, 1],
                 'k+', alpha=0.1)
        ...
        plt.tight_layout()
        plt.savefig('OPTICS-demo-' + outfile + '.png', format='png', bbox_inches='tight') # PNG
    ...
    CSV_OUT_filename = 'Clusters-OPTICS-' + outfile + '.csv'
    ...
    plt.plot(data[clust.labels_ == -1, 0],
             data[clust.labels_ == -1, 1],
             'k+', alpha=0.1)
    ...
    plt.savefig('OPTICS-' + outfile + '.png', format='png', bbox_inches='tight') # PNG
