...
def GenerateDBSCAN(filename, outfile="NULL", separator=";", XSize=10, YSize=9, dpi=300):
    ...
    param_k = 3                     # (dimension * 2) - 1
    param_min_samples = param_k + 1  # k + 1 || Best to use is 4 for 2D
    param_eps = 0.035                 # 1st valley || Max curvature of NearestNeighbors
    ...
    ## PHP-3-next :
    ## Directe : eps=0.035
    ## S=M : eps=0.05   S=B : eps=0.07   S=H : eps=0.09
    
    ## PHP-no-java-no-code :
    ## Directe : eps=0.04
    ## S=M : eps=0.05   S=B : eps=0.08   S=H : eps=0.13
    ##
    ## PHP-no-java :
    ## Directe : eps=0.04
    ## S=M : eps=0.045   S=B : eps=0.088   S=H : eps=0.09
    
    ## PHP-Courses Dissimilarity Simple :
    ## Directe : eps=0.04
    ## S=M : eps=0.06   S=B : eps=0.10   S=H : eps=0.10


    ## TENTATIVES SUR LES DONNEES NON MDS
    ...
    plt.plot(distances)
    ## Save into a PNG
    plt.savefig('NearestNeighbors-MDS+DBSCAN-' + outfile + '.png', format='png', bbox_inches='tight') # PNG
    ...
    db = DBSCAN(eps=param_eps, min_samples=param_min_samples, metric="euclidean")  # MDS => euclidean

    ## Apprentissage
    out_data = db.fit(points) # data = init data ; points = MDS points
    ...
    CSV_OUT_filename = 'Clusters-MDS+DBSCAN-' + outfile + '.csv'
    ...
    plt.title("MDS+DBSCAN (Automatic Clustering)\n"
    ...
    #plt.show()
    plt.savefig('MDS+DBSCAN-' + outfile + '.png', format='png', bbox_inches='tight') # PNG
