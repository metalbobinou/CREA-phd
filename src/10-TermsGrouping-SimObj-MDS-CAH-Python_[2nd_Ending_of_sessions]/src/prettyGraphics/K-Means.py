...
def GenerateMDS(filename, outfile="NULL", separator=";",
    ...
    in_data = pandas.read_table(filename, ";", header=0, index_col=0)
    in_labels = numpy.array(in_data.columns.values.tolist())
    ...
    print("## Optimisation du nombre de clusters : maximisation du coefficient silhouette")
    print("")
    ...
    for K in range(min, max): # max + 2 # 2 et 7 # min et max +2 (mais marche pas :( )
        kms = KMeans(n_clusters=K, random_state=1).fit(data)
        sil[K - 2] = silhouette_score(data, kms.labels_, metric='euclidean')
    ...
    #plt.show()
    plt.savefig('Coeff-Silhouette-K-Means-' + outfile + '.png', format='png', bbox_inches='tight')
    ...
    plt.scatter(in_values[:,0], in_values[:,1], color='silver',s=100)
    ...
    #plt.show()
    plt.savefig('Barycentres-K-Means-' + outfile + '.png', format='png', bbox_inches='tight') # PNG
    #plt.savefig(outfile + '.jpg', format='jpg', bbox_inches='tight') # JPG
    ...
    CSV_OUT_filename = 'Clusters-K-Means-' + outfile + '.csv'
...
'whitesmoke':           '#F5F5F5', 'yellow':               '#FFFF00',
    ...
    ax = plt.axes([0,0,2,2])
    ax.set_aspect(aspect='equal')
    plt.scatter(in_values[:,0],in_values[:,1], color=couleurs_groupes, s=150)
    for i in range(data.shape[0]):
        ax.annotate(data.index[i], (in_values[i,0], in_values[i,1]), color='black')
    #plt.show()
    plt.savefig('Clusters-K-Means-' + outfile + '.png', format='png', bbox_inches='tight')
    #plt.savefig(outfile + '.jpg', format='jpg', bbox_inches='tight') # JPG


    ## LIENS AMIS MARCHENT PAS BIEN AVEC K-MEANS SEUL
    if (False):
        ...
        #plt.show()
        plt.savefig('Liens-Amis-K-Means-' + outfile + '.png', format='png', bbox_inches='tight') # PNG
        #plt.savefig(outfile + '.jpg', format='jpg', bbox_inches='tight') # JPG
