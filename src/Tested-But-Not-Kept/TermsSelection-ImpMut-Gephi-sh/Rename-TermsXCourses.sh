...
function MyUsage()
{
    ...
    echo "[line1] X ; Term 1 ; Term 2 ; ..."
    echo "[line2+] Document ; 0 ; 1 ; ..."
    echo ""
}
...
do
	...
	# Transforms each lines (1st col) for putting correct documents names
	#${SED} -E 's/;[^;]*(C[0-9]+)\.csv/;\1/g' "${INPUT_NAME}" > ${TMP1_OUT}
	${AWK} -v FS=";" -v OFS=";" -v SUB_OFS="," \
	       -f rename-lines-courses.awk \
	       "${INPUT_NAME}" > ${TMP1_OUT}

	# Transforms 1st line (each cols) for putting correct terms (removes bn:ID)
	${AWK} -v FS=";" -v OFS=";" -v SUB_OFS="," \
	       -f rename-cols-bnID.awk \
	       ${TMP1_OUT} > ${TMP2_OUT}
