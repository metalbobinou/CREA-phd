...
# - If a fragment is linked to a part, it is kept for this part
...
#    }
	...
	if (NB_PARTS != 0)
	{
	    ...
	    for (i = 7; i <= NF; i++)
	    {
		CUR_PART = $i + 1; # Avoid part "0" for awk
		# Store in an array each fragment linked with each part
		# PARTS[CUR_PART] = CONCEPT_ID; # Require last pos...
		add_in_array_1_index(CONCEPT_ID, PARTS, CUR_PART);
