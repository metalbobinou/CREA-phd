...
OUTPUT_PREFIX=ordered-majority_

# Merge clusters and parts with a "majority" strategy :
# Each term vote for some parts. The most elected parts are kept.
# A part is considered as elected when appearing in more than half of the terms
#
...
${AWK} -v FS=";" -v OFS=";" -v SUB_OFS="," \
       -f merge-order-and-clusters-majority-plus-infos.awk  \
       ${TMP2_ORDERING_SORTED} ${TMP2_CLUSTERS_IN} > ${TMP3_CLUSTERS_COMPUTED}
...
# CLUSTERS_ORDER :
# Concept ID ; Significance ; "TERMS"/"PARTS" ; Nb Parts ; [ Part 1 , Part 2 , ... ] ; bn:ID 1 [; bn:ID 2 ; ... ]

# Clean the redundant number of parts
${AWK} -v FS=";" -v OFS=";" -v SUB_OFS="," \
       -f clean-redundant-parts-plus-infos.awk  \
